...
public class StatementIT
{
    ...
    public void shouldRunWithResult() throws Throwable
    {
        ...
        List<Record> result = session.run( "UNWIND [1,2,3] AS k RETURN k" ).asList();

        // Then the result object should contain the returned values
        assertThat( result.size(), equalTo( 3 ) );

        // And it should allow random access
        assertThat( result.get( 0 ).value( "k" ).javaLong(), equalTo( 1l ) );
        assertThat( result.get( 1 ).value( "k" ).javaLong(), equalTo( 2l ) );
        assertThat( result.get( 2 ).value( "k" ).javaLong(), equalTo( 3l ) );
    ...
    public void shouldRunParameterizedWithResult() throws Throwable
    {
        // When
        List<Record> result =
                session.run( "UNWIND {list} AS k RETURN k", parameters( "list", asList( 1, 2, 3 ) ) ).asList();
